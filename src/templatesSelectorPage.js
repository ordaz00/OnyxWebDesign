...
function TemplatesSelectorPage(props){
    let [checked, setChecked] = React.useState(false);
    let [css] = useStyletron();
    
    function pageChange(){
      props.updatePage("questions");
    }
    if(!checked){
      return(
        <div className='tabs'>
      ...
      return(
        <div className='tabs'>
            <HeaderNavigation className={css({width: "99.9%", borderStyle: "solid", borderWidth: "1px", borderBottomColor: "black", position: "sticky", top: "0"})}>
              ...
              <StyledNavigationList $align={ALIGN.center} className={css({paddingLeft: "0px !important", paddingRight: "0px !important"})}>
                <StyledNavigationItem className={css({display: "flex", flexDirection: "row", justifyContent: "center", alignItems: "center"})}>
                   <Checkbox
                    checked={checked}
                    checkmarkType={STYLE_TYPE.default}
                    onChange={e => setChecked(e.target.checked)}
                  ...
                  <Button
                      ...
                      size={SIZE.compact}
                      shape={SHAPE.default}
                      className={css({fontSize: "14px", marginLeft: "20px"})}
                    >
                      Click to Continue
                    </Button>
                </StyledNavigationItem>
...
export default TemplatesSelectorPage;